...
function _getId(start: number = 1) {
    let i = start;
    return () => i++;
}
...
let app_state = {
    root_node: undefined as unknown as NNode,
    root_x: 0,
    root_y: 0,
    scale: 1,
    projects: [] as string[],
    ...
    all_nodes: {} as { [k: number]: NNode }, // id: node
};

function nodeCreateDefault(parent: NNode | null): NNode {
    const new_node: NNode = {
        id: getId(),
        pr: parent,
        co: "",
        changed: true,
...
function nodeIsRoot(node: NNode): boolean {
    return node.pr == null;
}

function nodeRemove(node: NNode) {
...
function getTextSize(text: string, font: string): Dim {
...
function nodeCalcSize(node: NNode): Dim {
...
function viewBoxString(): string {
    const vb = app_state.view_box;
    return `0 0 ${vb.width} ${vb.height}`;
}

function render() {
    const svg = document.getElementById("main-svg")! as unknown as SVGElement
...
function calcBranchSizeRecursive(node: NNode): Dim {
    nodeCalcSize(node);
    let branch_dim: Dim = { w: 0, h: 0 };
...
function renderNode(
    svg: SVGElement /*HTML SVG Elem*/,
    node: NNode | null = null,
    pos: Pos = { x: 0, y: 0 }, // Center
    parent_pos: Pos = { x: 0, y: 0 } // Center
) {
    if (!node) node = app_state.root_node;
...
function renderNode_(
    ...
    let group_svg = undefined as unknown as SVGElement;
    let line_svg = undefined as unknown as SVGElement;
    ...
    if (line_svg) {
        line_svg.setAttribute(
            "d",
            `M${parent_pos.x} ${parent_pos.y} L${node_x} ${pos.y}`
        );
    }
...
function _serializedNodeToNNodeTmp(snode: SerializedNode): NNode {
    return {
        id: snode.id,
        pr: null,
        co: snode.co,
...
function fromJson(json_: string): NNode | null {
    try {
        let maxx = 0
        let root_node_idx: number | null = null;
        const parsed: SerializedNode[] = JSON.parse(json_);
        const tmp: NNode[] = parsed.map(_serializedNodeToNNodeTmp)
        const childs: { [k: number | null]: NNode[] } = {};
...
function getSaveKey(proj: string, name: string): string {
    return `${proj}_${name}`;
}
...
function save() {
    ...
    localStorage.setItem(
        getSaveKey(SAVES_STORAGE_KEY, proj_name),
        JSON.stringify(app_state.saves)
    );
...
function load() {
    const proj_name = (document.getElementById("proj_name")! as HTMLInputElement)
        .value;
    const save_name = (document.getElementById("save_name")! as HTMLSelectElement)
        .value;

    const loaded_root = fromJson(
        localStorage.getItem(getSaveKey(proj_name, save_name)) ?? ""
    );
